...
unaveraged_weights = dict()
for j in np.arange(min_return,max_return,stride):
    unaveraged_weights[j] = np.zeros(num_of_stocks)
...
for i in np.arange(min_return,max_return,stride):
    ...
    unaveraged_weights[i] = np.array(list(ef.clean_weights().values()))
...
for i in range(0,T):
    ...
    for j in np.arange(min_return,max_return,stride):
        try:
            ...
            history[j] += [np.array(list(cleaned_weights.values()))]
            acc[j] += 1
...
ax.plot(np.array(c_sig), np.array(c_mu),label='Distanced Boostrap PERF')
